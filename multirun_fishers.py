...
# multithreaded version of fisher matrix code
#  **multithreading sucks** because of the global interpreter lock (GIL) issue
#   -- not good for compute limited python coded
# **USE multirunGood_fishers.py INSTEAD*** -- this uses multiprocessing module.
# 
...
def run_all_fishers(do_constSurfBright=False):
    ...
    def run_one_fisher(inqueue):
        while True:
            this_info = inqueue.get()
    ...
    for ii in range(nthreads):
        ...
        calcOneFile = Thread(target=run_one_fisher,args=(inqueue,))
        calcOneFile.setDaemon(True)
        # sleep is a mystery-
        time.sleep(0.2)
        calcOneFile.start()
